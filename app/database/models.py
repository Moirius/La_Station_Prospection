"""
Modèles de données pour les leads/prospects
"""
...
class Lead(db.Model):
    ...
    business_type = db.Column(db.String(100), nullable=True)  # Type réel retourné par Google Places
    ...
    facebook_stats = db.Column(db.Text, nullable=True)  # Phrase brute likes/followers
    ...
    ai_extraction_status = db.Column(db.String(50), default='en_attente')  # en_attente, en_cours, succès, erreur
    ai_extraction_log = db.Column(db.Text, nullable=True)
    
    # Analyse IA du site web
    ai_analysis = db.Column(db.JSON, nullable=True)  # Stockage de l'analyse IA complète
    
    # Statut et logs
    statut_scraping = db.Column(db.String(50), default='en_attente')  # en_attente, en_cours, succès, erreur, captcha
    ...
    google_maps_telephone = db.Column(db.String(255), nullable=True)
    google_maps_adresse = db.Column(db.Text, nullable=True)
    
    # Données Site Web (IA)
    site_web_email = db.Column(db.String(255), nullable=True)
    site_web_telephone = db.Column(db.String(255), nullable=True)
    ...
    instagram_stats = db.Column(db.String(255), nullable=True)
    ...
    # Analyse IA complète (JSON) - Déjà défini plus haut
    
    def __repr__(self):
        return f'<Lead {self.nom}>'
    ...
    def update_ai_log(self, message):
        """Ajouter un message au log d'analyse IA"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        log_entry = f"[{timestamp}] {message}\n"
        
        if self.ai_extraction_log:
            self.ai_extraction_log += log_entry
        else:
            self.ai_extraction_log = log_entry
    ...
    def set_statut(self, statut):
        """Définir le statut de scraping"""
        self.statut_scraping = statut
        self.updated_at = datetime.utcnow()
    
    def set_ai_status(self, status):
        """Définir le statut d'analyse IA"""
        self.ai_extraction_status = status
        self.updated_at = datetime.utcnow()
    ...
    def update_has_info(self, info_type, has_info=True):
        """Mettre à jour si on a récupéré une information"""
        info_fields = {
            'email': 'has_email',
            'phone': 'has_phone',
            'address': 'has_address',
        ...
        if info_type in info_fields:
            setattr(self, info_fields[info_type], has_info)
            self.updated_at = datetime.utcnow()
            return True
        return False 